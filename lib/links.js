...
// Anchor text that is itself a URL/domain (e.g. someone pasted a bare link as the
// visible text). Rewriting these WOULD create a real anchor/href mismatch — the
// visible domain would no longer match the tracked href — which is exactly what
// spamcheck.js's link_mismatch rule is designed to catch. Everything else (normal
// text links like "Book a meeting") is safe to rewrite.
const LOOKS_LIKE_URL = /^(https?:\/\/|www\.)/i;

/** Links we must never rewrite: unsubscribe, the pixel, unresolved tokens, and
 *  anchors whose visible text is itself a URL. */
function shouldSkip(url, appUrl, label) {
  if (!url) return true;
  const u = url.trim();
  ...
  if (label && LOOKS_LIKE_URL.test(label.trim())) return true; // avoid anchor/href mismatch
  return false;
}
...
export function rewriteLinks({ html, sendId, appUrl }) {
  ...
  const out = String(html).replace(
    ...
    (match, pre, q, url, post, inner) => {
      // Anchor text, stripped of tags, gives the click report a human label — and
      // lets shouldSkip tell a real text link from a pasted bare URL.
      const label = inner
        .replace(/<[^>]*>/g, ' ')
        .replace(/&rarr;|&#8594;/gi, '')
        .replace(/&nbsp;|&#160;/gi, ' ')
      ...
      if (shouldSkip(url, appUrl, label)) return match;
