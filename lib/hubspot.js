...
export async function getOwnerAndDealName(contactId) {
  ...
  let batch;
  try {
    batch = await read([...REQUIRED, 'location', 'lead_region']);
  } catch {
    // `location` / `lead_region` aren't properties in every portal — if the read rejects
    // them, retry with just the required set so owner resolution never breaks.
