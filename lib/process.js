...
import { getDealOwnerId, getOwnerAndDealName, updateContact, logEmailToTimeline, getLoggedCallCount } from './hubspot.js';
...
export async function processContact(contact, campaigns, ownerMap) {
  ...
  const { region, stamped, resolved } = await resolveRegion({
    stampedRegion: p.lead_region, zip: p.zip_code, fallbackZips: [primaryDeal.location, primaryDeal.dealName]
  });
  if (!stamped && resolved) { try { await updateContact(contact.id, { lead_region: region }); } catch { /* non-fatal */ } }
