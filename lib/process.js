...
async function processContactCore(contact, campaigns, ownerMap) {
  ...
  //    everything below reads subject/body from `effectiveStep` instead. Variants only
  //    apply to plain email steps, never SMS or marketing-design steps. ──
  let variant = null;
  let effectiveStep = step;
  if (channel === 'email' && Array.isArray(step.variants) && step.variants.length && step.format !== 'design') {
    variant = pickVariant(contact.id, p.dw_campaign, stepIndex, step.variants);
    effectiveStep = { ...step, subject: variant.subject, body: variant.body };
  }
