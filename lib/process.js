...
import { bump as bumpMetric, registerLinks } from './metrics.js';
...
export async function processContact(contact, campaigns, ownerMap) {
  ...
  if (channel === 'sms' && !p.phone) {
    await logEvent({ type: 'error', contact: email || contact.id, campaign: p.dw_campaign, step: stepIndex + 1, channel,
                     detail: 'SMS withheld — contact has no phone number (check `phone` vs `mobilephone`)' });
    return { status: 'error', detail: 'contact has no phone (SMS step)' };
  }
  ...
  if (!gate.ok) {
    ...
    const at = nextOpenAt(region, campaign.window, step.days);
  ...
  if (channel === 'sms') {
    ...
    if (!ownerId) {
      await logEvent({ type: 'error', ...base, detail: 'SMS withheld — no deal owner id, so no number map lookup possible' });
      return { status: 'error', detail: 'SMS step needs a deal owner (no sendAs number map)' };
    }
    if (!smsNumberId) {
