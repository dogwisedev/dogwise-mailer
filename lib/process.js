...
export async function processContact(contact, campaigns, ownerMap) {
  ...
  const advance = async () => {
    ...
    if (isLast || step.delayDaysAfter == null || step.delayDaysAfter === 0) {
      // 0 delay = make it due immediately (next cron will pick it up)
      return updateContact(contact.id, { 
        dw_next_send: String(Date.now() - 60000), 
        dw_campaign_step: String(nextStepNum) 
      });
    } else {
      const delayMs = step.delayDaysAfter * 24 * 60 * 60 * 1000;
