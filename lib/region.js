...
export async function resolveRegion({ stampedRegion, zip, fallbackZips = [] }) {
  if (REGIONS.includes(stampedRegion)) return { region: stampedRegion, stamped: true, resolved: true };
  let z = extractZip(zip);
  if (!z) for (const f of fallbackZips) { z = extractZip(f); if (z) break; }
  // No ZIP found anywhere (contact or deal name) → region genuinely unknown.
  // Return a default for the EMAIL send-window timezone only; resolved:false tells the
  // SMS path NOT to text (we won't guess a number).
  if (!z) return { region: DEFAULT_REGION, stamped: false, resolved: false };
  const state = await zipToState(z);
  if (!state) return { region: DEFAULT_REGION, stamped: false, resolved: false };
  return { region: stateToRegion(state), stamped: false, resolved: true };
}
 ...
 *   window: { startMins, endMins }  — minutes since midnight; also accepts legacy
 *                                      { startHour, endHour }. Defaults 9:00–16:00.
 *   days:   { weekday, weekend }    — omitted => both allowed (back-compat)
 */
function edgeMins(window, key, fallback) {
  const m = window[`${key}Mins`];
  if (Number.isFinite(m)) return m;
  const h = window[`${key}Hour`];
  if (Number.isFinite(h)) return h * 60;
  return fallback;
}

export function evaluateWindow(t, window = {}, days) {
  const startMins = edgeMins(window, 'start', 9 * 60);
  const endMins   = edgeMins(window, 'end',  16 * 60);
  ...
  if (t.mins < startMins || t.mins >= endMins) {
    return { ok: false, reason: `outside ${fmt(startMins)}–${fmt(endMins)} (local ${fmt(t.mins)})` };
  }
  return { ok: true };
}
...
/**
 * When does this region's window NEXT open? Returns an epoch ms timestamp.
 *
 * WHY THIS MATTERS. getDueContacts sorts by dw_next_send ASCENDING, and a deferral used
 * to leave dw_next_send alone. A contact whose window was shut therefore kept its old
 * timestamp and sat permanently at the FRONT of the queue, re-read on every single cron
 * pass. Enough of them and they consume the whole per-run budget, so contacts that could
 * actually send are never reached. Rescheduling to the next opening lets a deferred
 * contact leave the queue and come back exactly when it is sendable.
 *
 * Scans forward in 15-minute steps for up to 8 days, which covers any window and any
 * weekday/weekend combination. Falls back to one hour ahead if nothing is found, so a
 * contact can never be parked indefinitely.
 */
export function nextOpenAt(region, window, days, fromMs = Date.now()) {
  const STEP_MS = 15 * 60 * 1000;
  const LIMIT = (8 * 24 * 60) / 15;
  const tz = timezoneForRegion(region);

  for (let i = 1; i <= LIMIT; i++) {
    const at = fromMs + i * STEP_MS;
    const parts = new Intl.DateTimeFormat('en-US', {
      timeZone: tz, weekday: 'short', hour: 'numeric', minute: 'numeric', hour12: false
    ...
    if (evaluateWindow(t, window, days).ok) return at;
  }
