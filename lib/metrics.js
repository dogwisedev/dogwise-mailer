 ...
 */
export async function countClick({ sendId, index, campaign, step, url, label, at = Date.now() }) {
  ...
  const r = await pipe([
    ...
    // Anchor text in a side hash, keyed by url. Kept separate so the existing click
    // counters keep their field format and no history is lost.
    ...(campaign && url && label ? [['HSET', `dwm:lklabel:${campaign}`, url, String(label).slice(0, 90)]] : [])
  ]);
  const n = Number(r?.[0] ?? 0);
...
export async function campaignLinks(campaign, limit = 20) {
  ...
  // Labels are strings, so read them raw rather than through the numeric hash helper.
  const rawLabels = await redis(['HGETALL', `dwm:lklabel:${campaign}`]) || [];
  const labels = {};
  for (let i = 0; i < rawLabels.length; i += 2) labels[rawLabels[i]] = rawLabels[i + 1];

  return Object.entries(o)
    .map(([field, clicks]) => {
      const idx = field.indexOf('|');
      const url = field.slice(idx + 1);
      return { step: Number(field.slice(0, idx)) || 0, url, label: labels[url] || '', clicks };
    })
...
/**
 * Best UTC hour to send, based on the campaign's own recorded open times so far.
 * Returns null until there's enough data (minOpens), so a fresh campaign keeps using
 * plain fixed-delay scheduling rather than optimizing off a handful of data points.
 * Called fresh at each scheduling decision — as more opens come in the picture updates
 * on its own, which is what gives the "reconfigure every N opens" behavior without a
 * separate counter to maintain.
 */
export async function optimalSendHour(campaign, minOpens = 20) {
  const hod = await campaignHourOfDay(campaign);
  const opens = hod.opened;
  if (!opens) return null;
  const total = opens.reduce((a, b) => a + b, 0);
  if (total < minOpens) return null;
  let bestHour = 0, bestCount = -1;
  opens.forEach((n, h) => { if (n > bestCount) { bestCount = n; bestHour = h; } });
  return { hour: bestHour, totalOpens: total };
}
...
// ── Send index, for time-based ("hasn't opened in X hours") rules ───────────
// Event-driven rules react to a pixel hit or a click. A NEGATIVE condition has no event
// to react to, so the cron has to sweep sends that are now old enough to judge. That
// needs an index of sendIds by time, which is what this sorted set is.
//
//   dwm:sidx:<campaign>   ZSET  score = sentAt ms, member = sendId
//   dwm:replied:<contactId>:<campaign>  marker so 'replies' is queryable per send

const SIDX_KEEP_DAYS = 30;

export async function indexSend(campaign, sendId, at = Date.now()) {
  if (!campaign || !sendId) return;
  const k = `dwm:sidx:${campaign}`;
  await pipe([
    ['ZADD', k, String(at), sendId],
    // Trim anything older than we would ever sweep, so the set stays small.
    ['ZREMRANGEBYSCORE', k, '0', String(at - SIDX_KEEP_DAYS * 86400000)]
  ]);
}

/**
 * sendIds whose age is between minAgeMs and maxAgeMs.
 * The upper bound stops us rescanning all history on every cron run; dedupe in
 * lib/triggers.js stops duplicate tasks within that window.
 */
export async function sendsAged(campaign, minAgeMs, maxAgeMs, limit = 40) {
  const now = Date.now();
  const r = await redis([
    'ZRANGEBYSCORE', `dwm:sidx:${campaign}`,
    String(now - maxAgeMs), String(now - minAgeMs),
    'LIMIT', '0', String(limit)
  ]);
  return Array.isArray(r) ? r : [];
}

export async function markReplied(contactId, campaign) {
  if (!contactId || !campaign) return;
  await redis(['SET', `dwm:replied:${contactId}:${campaign}`, String(Date.now()), 'EX', String(200 * 86400)]);
}

export async function hasReplied(contactId, campaign) {
  const r = await redis(['GET', `dwm:replied:${contactId}:${campaign}`]);
  return Boolean(r);
}

/** opens / clicks / replies for one send, as the rule evaluator wants them. */
export async function sendStats(sendId, contactId, campaign) {
  const [o, c, replied] = await Promise.all([
    getOpenCount(sendId),
    getClickCount(sendId),
    contactId && campaign ? hasReplied(contactId, campaign) : Promise.resolve(false)
  ]);
  return { opens: o.n || 0, clicks: c.total || 0, replies: replied ? 1 : 0 };
}
