...
/**
 * True if `body` looks like a full HTML document/fragment rather than the plain-text +
 * light-markdown syntax renderHtml() expects. renderHtml() HTML-escapes its input before
 * doing anything else, so real markup pasted into a plain step's `body` renders as
 * literal `&lt;table&gt;` text to the recipient instead of an actual table \u2014 it doesn't
 * error, it just silently mangles the email. This is the guard that stops it: a step
 * needs `format: 'design'` (create_marketing_design + attach_marketing_design, or the
 * dashboard builder) to send real HTML, not a body field on a plain email/SMS step.
 */
export function looksLikeFullHtml(body) {
  return /<!doctype\s+html|<html[\s>]|<body[\s>]|<table[\s>]|<td[\s>]|<tr[\s>]|<div[\s>]/i.test(String(body || ''));
}
