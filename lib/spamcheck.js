...
export function checkBody({ html, text, footerAdded = true, ownDomain = '' }) {
  ...
  const hiddenBlocks = (H.match(/<[^>]+style="[^"]*(display\s*:\s*none|font-size\s*:\s*0|visibility\s*:\s*hidden)[^"]*"[^>]*>([\s\S]{25,}?)<\//gi) || [])
    .filter(chunk => !/width=["']?1["']?/i.test(chunk));
  if (hiddenBlocks.length) {
    add(HIGH, 'hidden_text',
      'There appears to be hidden text (display:none, font-size:0 or visibility:hidden wrapping real content).',
      'Hidden text is treated as deliberate filter evasion and is one of the fastest ways to get a domain flagged. If a raw-HTML block came from another template, check it.');
  }
