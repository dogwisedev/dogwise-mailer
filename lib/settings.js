 ...
 * Falls back to the owner's East Coast number if the exact region isn't mapped.
 * Returns '' if the owner has no numbers configured.
 */
export async function smsNumberFor(ownerId, region) {
  const s = await getSettings();
  const forOwner = s.smsNumbers?.[String(ownerId)] || null;
  if (!forOwner) return '';
  return String(forOwner[region] || forOwner['East Coast'] || '').trim();
}
