...
const DEFAULTS = { senderLinks: {}, senderLinkFallback: '', smsNumbers: {}, placeholders: [], programLinks: {}, programLinkFallback: '' };
...
/**
 * Maps a program NAME (the value already sitting in type_of_program_sold) to the page on
 * your site that explains it — maintained once here, rather than as a property you'd have
 * to fill in correctly on every single deal. type_of_program_sold is presumably a fixed
 * dropdown, so a handful of entries covers every deal automatically, past and future.
 * Matched trimmed + case-insensitive, so "Board & Train " and "board & train" both hit.
 */
export async function programLinkFor(programName) {
  const s = await getSettings();
  const key = String(programName || '').trim().toLowerCase();
  if (!key) return s.programLinkFallback || '';
  for (const [name, url] of Object.entries(s.programLinks || {})) {
    if (String(name).trim().toLowerCase() === key) return url;
  }
  return s.programLinkFallback || '';
}
...
/** The global placeholder registry (see lib/tokens.js for the row shape). */
export async function getPlaceholders() {
  const s = await getSettings();
  return Array.isArray(s.placeholders) ? s.placeholders : [];
}
